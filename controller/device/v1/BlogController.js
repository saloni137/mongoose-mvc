...
const findAllBlog = async (req,res) => {
  try {
    ...
    if (options.select && options.select.length){
      options.select = [ 'isActive', 'createdAt', 'updatedAt', 'name', 'userType' ].filter(Set.prototype.has, new Set(options.select));
    } else {
      options.select = [ 'isActive', 'createdAt', 'updatedAt', 'name', 'userType' ];
    }
...
const getBlog = async (req,res) => {
  try {
    ...
    let options = {};
    let selectedFields =  [ 'isActive', 'createdAt', 'updatedAt', 'name', 'userType' ];
    selectedFields = utils.getSelectObject(selectedFields);
 ...
 * @return {Object} : response of fileUpload {status, message, data}
 */
const fileUpload = async (req,res)=>{
  try {
    ...
    let uploadedFiles = await fileUploadService.upload(fileUpload);
  } 
  catch (error){ 
 ...
 * @param {Object} req : request
 * @param {Object} res : response 
 * @return {Object} : response of testAction {status, message, data}
 */
